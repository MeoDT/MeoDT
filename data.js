
const softwareData = [
  {
    id: 1,
    name: "MeoKL",
    description: "A modern Roblox executor for smooth and secure script execution.",
    icon: "fas fa-image",
    features: [
      "Injecting",
      "Online virus-free scripts"
    ],
  },
  {
    id: 2,
    name: "Cloudstorage",
    description: "Reliable global cloud storage for your files.",
    icon: "fas fa-image",
    features: [
      "Storage all over the world",
      "Fast downloading",
      "! Only possible when enough support is there, over time it will get more !"
    ],
  },
  {
    id: 3,
    name: "MeoKLient",
    description: "A flexible Minecraft launcher and client with extra tools and custom options.",
    icon: "fas fa-laptop-code",
    features: [
      "Offline and online support",
      "Offline skins",
      "Easy self-host server",
      "Free cosmetics",
      "Custom capes"
    ],
  },
  {
    id: 4,
    name: "Piratebay",
    description: "A digital hub for accessing free games, music, movies and indie content. All downloaded from Piratewebsites, secured through Virustotal",
    icon: "fas fa-lock",
    features: [
      "Free games, music, movies and more!",
      "Auto antivirus scan",
      "Indie games with ads purchases available"
    ],
  },
];

const aiResponses = {
  hgai: "hello, im not a ai as you know it. im not very smart",
  help: "hello, hi, free, download, update, function, thanks, bye, default, support, bug, features, license, account, login, signup, forgotpassword, contact, roadmap, team, donate",
  hello: "Hello! How can I help you with MeoDT Software?",
  hi: "Hi! How can I assist you?",
  free: "All our software is completely free. You can find it in the Software tab.",
  download: "You can download our software in the Software tab.",
  update: "Software updates are released regularly. Check the Software tab for the latest versions.",
  function: "Our software includes all functions for free.",
  thanks: "You're welcome! Can I help you with anything else?",
  bye: "Goodbye! Feel free to return if you have more questions.",
  default: "Sorry, I didn't understand that. write 'help' to see all the things i can understand ",
  support: "For technical support, please visit the Support tab or contact us via the Help section.",
  bug: "If you encounter a bug, please report it through our Bug Report form in the Support tab.",
  features: "You can find a list of all available features in the Software tab under each product.",
  license: "All our software is licensed under our standard Terms of Use, available in the Legal tab.",
  account: "You can manage your account settings in the Account tab.",
  login: "Please log in through the Login button at the top right corner.",
  signup: "You can create an account by clicking on the Sign Up button at the top right.",
  forgotpassword: "If you forgot your password, use the Forgot Password link on the login page to reset it.",
  contact: "For general inquiries, please use our Contact form in the Support tab.",
  roadmap: "Our development roadmap is available in the About tab to keep you updated on upcoming features.",
  team: "You can learn more about our team in the About tab.",
  donate: "You can support our work through the Donate tab. Every contribution helps us keep the software free!"
};

const newsData = [
    {
    id: 4,
    date: "23.7.25",
    title: "Dark and Whitemode added",
    content: "No Flashing anymore",
  },
  {
    id: 3,
    date: "23.7.25",
    title: "Deleted premium",
    content: "No more Premium.",
  },
  {
    id: 2,
    date: "23.7.25",
    title: "Website got setup",
    content: "The website got released",
  },
  {
    id: 1,
    date: "22.7.25",
    title: "Website got updatet completly",
    content: "Better Ai and overall the intire webside",
  },
];
